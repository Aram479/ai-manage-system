...
const ChatConversation = forwardRef(
  (props: ChatConversationProps, ref: Ref<IChatConversationRef>) => {
    const { chat, onSendMessage, isConnected } = props;
    const { confirmRole, updateSelectRole, updateConfirmRole } =
      useAgentRoleContext();
    const { userInfo } = useModel("user");
    const [conversationList, setConversationList] = useState<
      ApiTypes.TConversationList[]
    >([]);
    ...
    const getConversationListReq = useRequest(getConversationListApi, {
      manual: true,
      onSuccess: (res) => {
    ...
    const renderDateSeparator = (message: any, index: number) => {
      if (!conversationList || !Array.isArray(conversationList)) return null;

      const currentDate = dayjs(message.timestamp).format("YYYY-MM-DD");
      const prevDate =
        index > 0 && conversationList[index - 1]
          ? dayjs(conversationList[index - 1].timestamp).format("YYYY-MM-DD")
          : "";
    ...
    useEffect(() => {
      if (chat.userId) getConversationListReq.run({ userId: chat.userId! });
    }, [chat.userId]);

    const updateConversation: IChatConversationRef["updateConversation"] = (
      newMessage
    ) => {
      conversationList.push(newMessage);
      ...
      <div className={styles.container}>
        ...
        <div className={styles.messagesContainer}>
          {!!conversationList.length ? (
            conversationList.map((msg, index) => renderMessageRow(msg, index))
          ) : (
...
const ChatConversationProvider = (
  ...
  ref: Ref<IChatConversationRef>
) => {
  const providerRef = useRef<IChatConversationRef>(null);
  useImperativeHandle(ref, () => ({
    updateConversation: (list) => {
      providerRef.current?.updateConversation(list);
    },
  }));
  return (
    <AgentRoleProvider>
      <ChatConversation {...props} ref={providerRef} />
    </AgentRoleProvider>
  );
};
export default forwardRef(ChatConversationProvider);
