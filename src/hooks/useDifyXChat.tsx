...
import { difyXRequest } from "@/services/dify.api";
...
interface IUseDifyXChat {
  userName?: string;
  aiName?: string;
  defaultMessage?: string;
  requestBody?: IDifyRequestBody;
  onSuccess?: (messageData: TResultStream) => void;
}
const useDifyXChat = (props: IUseDifyXChat) => {
  ...
  const chatRequest: XAgentConfigCustom<TResultStream>["request"] = async (
    ...
    const requestData = {
      ...
      query: messagesData.message?.chatContent,
      user: userRole,
    };
    await difyXRequest.create(
      requestData,
      {
        // 请求结束后调用
        onSuccess: (res) => {
          if (res) {
            ...
            if (!isStreamLocked.current) {
              const conversationId = processorRef.current.getConversationId();
