...
import useDifyXChat from "@/hooks/useDifyXChat";
...
const ChatCmp = (props: IChatCmpProps, ref: Ref<TChatRef>) => {
  ...
  const defaultRequestConfig = useMemo<{
    requestBody: Partial<IDifyRequestBody>;
  }>(() => {
    const toolsProps = {
      menuList,
      userMenus,
  ...
  // 通义千问
  const Ai_Dify = useDifyXChat(defaultRequestConfig);
  // 当前切换的ai
  const Ai_Primary = Ai_Dify;
